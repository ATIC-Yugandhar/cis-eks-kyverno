...
SUMMARY_REPORT="$REPORT_DIR/noncompliant-summary.md"
...
terraform -chdir="$NONCOMPLIANT_DIR" init -input=false -no-color > /dev/null

echo -e "${YELLOW}🚀 [50%] Applying noncompliant configuration...${NC}"
terraform -chdir="$NONCOMPLIANT_DIR" apply -auto-approve -input=false -no-color
...
# 📊 Noncompliant Configuration Validation Summary
...
| **Configuration Type** | Noncompliant |
| **Status** | ✅ Complete |
...
echo -e "${RED}✅ [100%] Validation complete!${NC}"
